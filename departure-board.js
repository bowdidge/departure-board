/* Departure board: Java / CSS3 implementation of rail station
   departure board.
   From http://github.com/paulcuth/departure-board

   Modified by Robert Bowdidge to get it to run ok on a Raspberry Pi.
*/
...
// Interval between starting each row.
var ROW_START_DELAY = 1000;  // was 2000.

// Interval between starting each colum,n.
var COLUMN_START_DELAY = 10; // was 25.

var prev_values = [];

/* Changes the message on the board. */
DepartureBoard.prototype.setValue = function (value) {
	...
	for (var r = 0, rl = this._letters.length; r < rl; r++) {
		...
		if (prev_values.length > r && prev_values[r] == value[r]) {
		    /* No need to redraw row.*/
                } else {
		    // For each row.
		    for (var i = 0, l = this._letters[r].length; i < l; i++) {
			// Start the changing of the value for each cell.
			(function (r, i) {
			    window.setTimeout (function () {
				    var letterValue = value[r].substr (i, 1) || '';
				    me._letters[r][i].setValue (letterValue);
				}, ROW_START_DELAY * r + COLUMN_START_DELAY * i + Math.random () * 400);
			})(r, i);
		    }
                }
	}
